class Api::StocksController < ApplicationController
  ...
  def balance
    ...
    focus = Point.all.map {|x| {id:x.chain_id,block:x.chain.block,amount: balances[x.chain.block] || 0.0,cost: x.chain.buy_business.map{|x| x.total }.sum,market:x.chain.market.first}}.unshift({block:'USDT',amount:balances['USDT']})
